...
def preferences() -> Response | str:
    ...
    interfaces = {"u1traspace": 0, "myspace93": 1, "twitter": 2}
    preferences_form: FlaskForm = forms.preferences_form(
        interface=interfaces[selected_interface], privacy=selected_privacy
    )
    ...
    properties.update({"css": profile.get_profile_css(session["user_id"])})
    ...
    if scheduled_tasks:
        for task in scheduled_tasks:
            avatar_uploading = (
                True if check_task(task, "image") is True else avatar_uploading
            )
            audio_uploading = (
                True if check_task(task, "audio") is True else avatar_uploading
            )
    if running_tasks:
    ...
    return render_template(
        ...
        forms={"preferences": preferences_form},
    )


def check_task(task: dict, type: str) -> bool:
    if (
        task["name"] == "tasks.transcoding._transcode_and_upload_image"
        and type == "image"
    ):
        return True
    elif (
        task["name"] == "tasks.transcoding.transcode_and_upload_audio"
        and type == "audio"
    ):
        return True
...
def form_handler(query: Query, preferences_form: FlaskForm) -> None:
    if len(preferences_form.bio.data) < 4096:
        ...
        preferences_form.bio.data = (
            None if preferences_form.bio.data == "" else preferences_form.bio.data
        )
        ...
        if file_extension in CONFIG.ALLOWED_IMAGE_UPLOAD_EXTENSIONS:
            ...
            if magic.from_file(file_path, mime=True) in CONFIG.ALLOWED_IMAGE_MIME_TYPES:
                tasks.transcode_and_upload_images.delay(
                    file_path, USERCONTENT_DIR, session["user_id"]
                )
    ...
    if css:
        tasks.save_css.delay(css, USERCONTENT_DIR, session["user_id"])
