...
class register_form(FlaskForm):
    ...
    username: StringField = StringField("username", validators=[
        ...
        Length("username", 2, 30)
    ])
    password: PasswordField = PasswordField("password", validators=[
        DataRequired("password"),
        Length("password", 2, 64)
    ])
    confirm: PasswordField = PasswordField("confirm", validators=[
        DataRequired("password confirmation"),
        Length("password confirmation", 2, 64),
        EqualTo("password", message=CONFIG.STRINGS["errors"]["validation"]["password_mismatch"])
    ])
    submit: SubmitField = SubmitField("submit")
...
class new_blog_form(FlaskForm):
    ...
    corpus = TextAreaField("corpus", validators=[
        DataRequired("corpus"),
        Length("corpus", 1, 4000)
    ])
...
class _string_input_form(FlaskForm):
    corpus = StringField("corpus", validators=[
            DataRequired("corpus"),
            Length("corpus", 1, 1024)        
        ])
    submit = SubmitField("submit")
