...
class _Query:
    ...
    def search_users(self, search_term: str) -> list:
        return users.search_users(self.cur, search_term=search_term)

    def insert_user(self, email: str, username: str, password: str) -> None:
        users.insert_user(self.cur, email=email, username=username, password=password)
    ...
    def update_user(
        ...
        users.update_user(
            self.cur, user_id=user_id, about=about, layout=layout, private=private
        )
    ...
    def insert_page_comment(self, page_id: int, author_id: int, corpus: str) -> None:
        page_comments.insert_page_comment(
            self.cur, page_id=page_id, author_id=author_id, corpus=corpus
        )

    def delete_page_comment(
        self, comment_id: int, soft: bool = True, limit: int = 1
    ) -> None:
        page_comments.delete_page_comment(
            self.cur, comment_id=comment_id, soft=soft, limit=limit
        )
    ...
    def search_blogs(self, search_term: str) -> list:
        return blogs.search_blogs(self.cur, search_term=search_term)

    def insert_blog(self, author_id: int, title: str, corpus: str) -> None:
        blogs.insert_blog(self.cur, author_id=author_id, title=title, corpus=corpus)

    def delete_blog(self, blog_id: int, soft: bool = True, limit: int = 1) -> None:
        blogs.delete_blog(self.cur, blog_id=blog_id, soft=soft, limit=limit)
    ...
    def insert_blog_comment(self, blog_id: int, author_id: int, corpus: str) -> None:
        blog_comments.insert_blog_comment(
            self.cur, blog_id=blog_id, author_id=author_id, corpus=corpus
        )
    ...
    def insert_friend(
        self, sender_id: int, recipient_id: int, approved: bool = False
    ) -> None:
    ...
    def select_user_conversations(
        ...
        return messages.select_user_conversations(
            self.cur, user_id=user_id, count=count, order=order, limit=limit
        )

    def insert_message(self, sender_id: int, recipient_id: int, corpus: str):
        messages.insert_message(
            self.cur, sender_id=sender_id, recipient_id=recipient_id, corpus=corpus
        )
