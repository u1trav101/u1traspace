    ...
    client_settings_from_dict(
        {
            ...
            "identityURL": os.getenv(
                "IDENTITY-URL", "https://identity.bitwarden.com/connect/token"
            ),
            "userAgent": "Python",
        }
    )
...
class CONFIG:
    ...
    MARIADB_PASSWORD: str = (
        client.secrets().get("bf0630d0-d130-479c-b8c4-b173001c9148").data.value
    )
    MARIADB_ROOT_PASSWORD: str = (
        client.secrets().get("e653a440-6388-4406-bd72-b17101589fe1").data.value
    )
    ...
    SECRET_KEY: str = (
        client.secrets().get("04d85160-cad6-42e6-9e4f-b17501735703").data.value
    )
    ...
    S3_SECRET_KEY: str = (
        client.secrets().get("e447c57f-6f12-4be6-bf02-b17501870a12").data.value
    )
    ...
    BLACKLISTED_SEARCHES: list[str] = safe_load(
        Path("config/search-blacklist.yml").read_text()
    )
    ALLOWED_IMAGE_UPLOAD_EXTENSIONS: list[str] = safe_load(
        Path("config/img-ext-whitelist.yml").read_text()
    )
    ALLOWED_IMAGE_MIME_TYPES: list[str] = safe_load(
        Path("config/img-mime-whitelist.yml").read_text()
    )
    ALLOWED_AUDIO_UPLOAD_EXTENSIONS: list[str] = safe_load(
        Path("config/audio-ext-whitelist.yml").read_text()
    )
    ALLOWED_AUDIO_MIME_TYPES: list[str] = safe_load(
        Path("config/audio-mime-whitelist.yml").read_text()
    )
