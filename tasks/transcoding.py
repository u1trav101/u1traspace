...
def _transcode_and_upload_image(file_path, usercontent_dir, user_id, size):
    os.system(f"ffmpeg -loglevel error -hide_banner -loop -1 -i {file_path} -y -vf scale={size}:{size} {usercontent_dir}img/rsz/{size}px/{user_id}.webp")
    cdn.upload_image(
...
def transcode_and_upload_images(file_path, usercontent_dir, user_id):
    ...
    os.system(f"ffmpeg -loglevel error -hide_banner -loop -1 -i {file_path} -y {usercontent_dir}img/raw/{user_id}.webp")
    cdn.upload_image(
...
def transcode_and_upload_audio(file_path, usercontent_dir, user_id):
    os.system(f"ffmpeg -loglevel error -loop -1 -i {file_path} -y {usercontent_dir}audio/{user_id}.mp3")
    cdn.upload_audio(
