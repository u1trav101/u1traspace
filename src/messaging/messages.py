...
class Messages:
    def __init__(self, sender_id: int, recipient_id: int) -> None:
        ...
        self.messages: list[dict] | None = poll_incoming_messages(
            sender_id, recipient_id, 0
        )
        self.last_id: int = self.messages[-1]["message_id"] if self.messages else 0
        self.last_index: int = (
            self.messages.index(self.messages[-1]) if self.messages else 0
        )
        self.is_new_messages: bool = False

    def refresh(self) -> None:
        new_msgs = poll_incoming_messages(
            self.sender_id, self.recipient_id, self.last_id
        )
